...
function SummarySection() {
  ...
  const isWithinRange = charCount >= SUMMARY_MIN_CHARS && charCount <= SUMMARY_MAX_CHARS;
  ...
  const loadSection = useCallback(
    async (targetToLoad: SaveTarget, force = false) => {
      ...
      try {
        ...
        console.error('Failed to load summary draft', err);
  ...
  return (
    <div className="cv-ai-shell">
      <Tabs value="summary" onValueChange={handleTabChange}>
        <TabsList>
          ...
          <section className="ai-section">
            <header className="ai-section__header">
              ...
              <p className="ai-section__description">
                自己PRやQ&Aの内容を踏まえて、200〜400文字の職務要約を作成します。文体は簡潔かつ客観的に整え、保存先を切り替えて管理できます。
              </p>
            </header>
            ...
            <div className="ai-section__editor">
              <label className="ai-label" htmlFor="summary-text">
                職務要約テキスト
              </label>
              <textarea
                id="summary-text"
              ...
              <div className="ai-section__meta">
                <span className={showRangeHint ? 'ai-meta ai-meta--alert' : 'ai-meta'}>
                  文字数: {charCount}（推奨 {SUMMARY_MIN_CHARS}〜{SUMMARY_MAX_CHARS}）
                </span>
            ...
            <div className="ai-section__controls">
              <label className="ai-label" htmlFor="summary-target">
                保存先
              </label>
              <select
                id="summary-target"
        ...
        <TabsContent value="selfpr">
          <section className="ai-section ai-section--note">
            <p>自己PRの編集に移動します。</p>
            <Button variant="secondary" onClick={() => router.push('/cv/2')}>
              自己PRタブへ
            </Button>
          </section>
        </TabsContent>
